...
const AdminOrder = () => {
  ...
  const readyOrders = orders.filter((order) => order.status === "ready for delivery");
  ...
  // Filtrar los pedidos
  const dineInOrders = readyOrders.filter((order) => order.orderType === "dine-in");
  const takeoutOrders = readyOrders.filter((order) => order.orderType === "takeout");
  const deliveryOrders = readyOrders.filter((order) => order.orderType === "delivery");
  ...
  return (
    <div className="text-white p-6 mt-[100px] flex flex-col items-center gap-6 w-full bg-[#111827]">
      ...
      <section className="w-full">
        <h2 className="text-2xl font-semibold mb-4 text-left">Cooking Orders</h2>
        <div className="border border-gray-700 p-4 rounded-lg flex gap-4 justify-center bg-yellow-500">
          {cookingOrders.length > 0 ? (
            ...
            <p className="text-center w-full text-white">No cooking orders available</p>
          )}
        </div>
      </section>
      ...
      <section className="w-full">
        <h2 className="text-2xl font-semibold mb-4 text-left">Ready for Delivery</h2>
          ...
          <div className="border border-gray-700 p-4 rounded-lg  bg-yellow-500">
            ...
            <div className="flex flex-wrap gap-4 justify-center">
              {dineInOrders.length > 0 ? (
                ...
                <p className="text-center w-full text-white">No dine-in orders ready</p>
              )}
            </div>
          </div>
          ...
          <div className="border border-gray-700 p-4 rounded-lg bg-yellow-500">
            ...
            <div className="flex flex-wrap gap-4 justify-center">
              {takeoutOrders.length > 0 ? (
                ...
                <p className="text-center w-full text-white">No takeout orders ready</p>
              )}
            </div>
          </div>
          ...
          <div className="border border-gray-700 p-4 rounded-lg bg-yellow-500">
            ...
            <div className=" flex flex-wrap gap-4 justify-center">
              {deliveryOrders.length > 0 ? (
                ...
                <p className="text-center w-full text-white">No delivery orders ready</p>
              )}
            </div>
          </div>
        </div>
  ...
  <table className="w-full bg-white border border-gray-300 shadow-lg rounded-t-[500px]"> {/* Añadir rounded-t-lg aquí */}
    <thead>
      <tr className="bg-yellow-500 border-b border-gray-300">
        <th className="p-2 border-r border-[#1F2937] text-black">First Name</th>
        <th className="p-2 border-r border-[#1F2937] text-black">Last Name</th>
        <th className="p-2 border-r border-[#1F2937] text-black">Date & Time</th>
        <th className="p-2 border-r border-[#1F2937] text-black">Products</th>
        <th className="p-2 text-black">Total</th>
      </tr>
    </thead>
    <tbody>
      {deliveredOrders.map((order, index) => (
        <tr key={index} className="bg-white border-b border-[#1F2937] hover:bg-gray-100">
          <td className="p-2 border-r border-[#1F2937] text-black">{order.firstName}</td>
          <td className="p-2 border-r border-[#1F2937] text-black">{order.lastName}</td>
          <td className="p-2 border-r border-[#1F2937] text-black">{order.dateTime}</td>
